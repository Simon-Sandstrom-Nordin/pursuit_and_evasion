...
%             disp("agent number: " + num2str(agent) + "y_index2: " + num2str(y_index))
            ...
            M(x_index, y_index) = agents(agent).color; % Use color to represent agents
            ...
            if length(agents(agent).x_index_memo)>50
                M(agents(agent).x_index_memo(1),agents(agent).y_index_memo(1)) = 0;
        ...
        if visualization_boolean
            ...
            for agent = 1:length(agents)
                ...
                if all(x_index-1 > 0) && all(x_index+1 <= N_matrix) && all(y_index-1 > 0) && all(y_index+1 <= N_matrix)
                    M(x_index, y_index) = 4; % Use color to represent agents
                end
            ...
            % disp(K)
            % Display the playing field matrix% Assuming M and agents(1).exploration_map are matrices representing your data
            
            % Plotting both images side by side
            % subplot(1,2,1); % Create subplot with 1 row, 2 columns, and select the first subplot
            imagesc(M); % Display the first image
            % title('imagesc(M)'); % Add a title for the first subplot
            % 
            % subplot(1,2,2); % Create subplot with 1 row, 2 columns, and select the second subplot
            % heatmap(agents(1).exploration_map); % Display the heatmap
            % title('Heatmap'); % Add a title for the second subplot
            
            % colorbar; % Add a colorbar
            
            % Adjust figure size if necessary
            % set(gcf, 'Position', [100, 100, 1000, 400]); % Adjust width and height as needed
            ...
            %title_str = ["All algorithms activated"];
            ...
            for agent = 1:length(agents)
                ...
                if any(x_index > 0) && any(x_index <= N_matrix) && any(y_index > 0) && any(y_index <= N_matrix)
                    M(x_index, y_index) = 0; % Use color to represent agents
                end
            end
        end
...
%         End the game if there are no remaining blue agents
        ...
        % if agents(length(agents)).y >= 72/N_matrix && ...
        %          agents(length(agents)).y <= 151/N_matrix && ...
        %          agents(length(agents)).x >= 122/N_matrix && ...
        %          agents(length(agents)).x <= 221/N_matrix
        %     reason_for_exit = "All evaders are dead";
        %     % writematrix(initial_matrix, "initial_matrix.csv")
        %     % writematrix(initial_M, "initial_m.csv")
        %     return;
